...
def callback(img_msg,pose,laser):
...
def listener():
    ...
    pose  = message_filters.Subscriber('/hsrb/wheel_odom',Odometry)
    symbol= message_filters.Subscriber('/hsrb/base_scan',LaserScan)
    ats= message_filters.ApproximateTimeSynchronizer([image,pose, symbol],queue_size=5,slop=.1,allow_headerless=True)
